...
class tinker_helper(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        self.resistor_output = tk.StringVar()
        self.resistance_list = tk.StringVar(value="Current Values: None")
        self.resistance_values = []
        self.band1 = tk.StringVar()
        self.band2 = tk.StringVar()
        self.band3 = tk.StringVar()
        self.res_mult = tk.StringVar()
        self.res_tolerance = tk.StringVar()
        # -~-~
        self.create_widgets()
    ...
    def create_resistor_tab(self, parent):
        ...
        """
        tk.Label(parent, text="Resistors").grid()

        self.resistor_value_entry = tk.Entry(
            parent,
            textvariable=self.resistor_value
            )
        
        self.resistor_value_entry.pack()

        tk.Button(
            parent,
            text="Add to current",
            command=self.add_resistor_to_list
        )
    ...
    def calculate_resistance(self):
        value_in = self.resistor_value_entry.get()
    ...
    def add_resistor_to_list(self):
        value_in = self.resistor_value.get()
        ...
        try:
            ...
            self.resistance_values.append(value_in)

            display_list = ["{:.2f}Ω".format(value for value in self.reistance_values)]
            self.resistance_list_display.set(", ".join(display_list))

            self.resistor_value.set("")
            self.resistor_output.set("Added {:.2f}Ω".format(value_in))

        except ValueError:
            self.resistor_output.set("Error")
